...
// 	DataUser           []User
// 	DataThread         []Thread
// 	DataThreadMessage  []ThreadMessage
// 	DataLike           []Like
// 	DataThreadCategory []ThreadCategory
...
func InsertIntoThread(title string, content string, created_at string) {
	db := InitDatabase("dataBase/forum.db")
	_, err := db.Exec(`INSERT INTO Thread (title, content, created_at) VALUES (?, ?, ?)`, title, content, created_at)
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
